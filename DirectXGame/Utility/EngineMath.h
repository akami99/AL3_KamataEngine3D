...
struct Ball {
	KamataEngine::Vector3 position;         // ボールの位置
	KamataEngine::Vector3 velocity;         // ボールの速度
	KamataEngine::Vector3 acceleration;     // ボールの加速度
	float mass;               // ボールの質量
	float radius;             // ボールの半径
...
struct Pendulum {
	KamataEngine::Vector3 anchor;                // アンカーポイント。固定された端の位置
	float length;                 // 紐の長さ
	float angle;                  // 現在の角度
